local save = require("legacylua.novel-save")
...
function M.add_files(opt, filenames)
		--local count = #script
		script = {}
		--for i=0, count do script[i]=nil end
		local loaded = load_script('/assets/novel/scripts/_novelglobals.txt')
		for k, line in pairs(loaded) do
			table.insert(script, line)
		end
		for f,file in pairs(filenames) do
			print("MSCRIPT:: LOAD FILE:: ",file)
			local loaded2 = load_script('/assets/novel/scripts/'..file..".txt")
