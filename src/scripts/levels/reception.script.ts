...
function load_adjacent() {
  ...
  for (const adjacent of roomNames) {
    ...
    for (station in stations) {
      const npc = stations[station]
      station == `${adjacent}desk`
        ? msg.post(`${adjacent}deskarea#station`, 'loadStation', {
            npc,
            adjacent,
          })
        : msg.post(`/${adjacent}${station}#npc_loader`, 'load_npc', { npc })
    }
    ...
    for (swap in swaps) {
      const npc = swaps[swap][1]
      print('RECEPTIONSWAPS::', swap, swaps[swap][0], swaps[swap][1])
      msg.post(`/${adjacent}${swaps[swap][0]}#npc_loader`, 'load_npc', { npc })
    }
  }
