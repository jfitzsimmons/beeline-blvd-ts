...
function script_builder(
  ...
  const paths: string[] = []
  //TOD START HER:: NOW that i have novel.priority
  //need to reorganize all this crap!!! TESTJPF

  //!!!!!!!!!!!!!!!

  //testjpf this doesnt chek if player ahs a caution
  //if npc has a caution/ does player want to talk to them about it?
  // QUEST YES
  // if player has caution, do you want to ? depends.
  //CONFRONT YES
  //This is why I have a specific confrontation function on level.script
  // if ![""]

  const caution = tasks.npc_has_caution('any', novel.npc.labelname)

  if (caution != null) {
    // testjpf concern being set forn interact gui overwritten
    // following is just used for quest?
    // If i talk to npc with theft cation its overwreitten.
    // would be coold to talk to them about their theft
    if (!['concern'].includes(novel.reason)) novel.reason = caution.reason

    novel.caution = { ...caution }
    print('novel.caution.label', novel.caution.label)
    // reasons not set up wi...th dialog
  ...
  //testjpf
  //following wont work because npc might have these cautions
  //needtocheckif for player or not
