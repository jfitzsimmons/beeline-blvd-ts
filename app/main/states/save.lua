...
-- sees like it just goes to next text line, 
--but can be sent instructions to fo to particular line
M.global.read_lines = {} 
M.global.var = {} --not used at all
...
function dump(o) --util this testjpf
...
function M.write(slot, quick, auto)
	--testjpf TODO:
	--import something from main.state like get npcstate
	-- ex: local state = states.get_state()
	local state = game
...
function M.load(slot)
	...
	if loaded then
		...
		--testjpf this can work.  mostly game is local.  you need a function that updates global.
		game = loaded_table
	...
	--return loaded
	return loaded -- testjpf
	-- with mnovel loaded is then used for logic that goes to correct line in script.
	-- we need to do similar but with room
end
